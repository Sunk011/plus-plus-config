...
Claude Code 使用 `~/.claude/settings.json` 的 JSON 配置，不使用 Codex 的 `~/.codex/config.toml`。`notify_claude_hook.js` 会读取 Claude Code hook 通过 stdin 传入的 JSON，并复用第 2 步的 PushPlus token 配置。

不要让 Claude Code hook 直接指向当前 Git 仓库目录。先把本仓库里的 skill 文件安装到 Claude Code 用户级 skills 目录，再让 hook 调用用户级脚本，这样换工作目录或删除当前 clone 后仍然可用。

推荐先启用 `Notification`，用于 Claude Code 需要权限、等待用户输入或需要关注时提醒；如果也希望每轮 Claude Code 停止时提醒，再启用 `Stop`。

### 7.1 安装到 Claude Code 用户级 skills 目录

Windows PowerShell 在本仓库根目录运行：

```powershell
$src = Resolve-Path ".\pushplus-turn-notify"
$dst = Join-Path $env:USERPROFILE ".claude\skills\pushplus-turn-notify"
...
Git Bash / macOS / Linux 在本仓库根目录运行：

```bash
src="$(pwd)/pushplus-turn-notify"
dst="$HOME/.claude/skills/pushplus-turn-notify"
...
安装后 Claude Code hook 使用：

```text
~/.claude/skills/pushplus-turn-notify/scripts/notify_claude_hook.js
```

### 7.2 写入 `~/.claude/settings.json`

如果 `~/.claude/settings.json` 已存在，把下面的 `hooks` 合并进已有 JSON；不要覆盖已有的 `env`、`enabledPlugins`、`statusLine` 等配置。

Windows / Git Bash 推荐配置：
...
如果只想在需要关注时提醒，不想每轮结束都提醒，可以删除 `Stop` 配置块。不要把 PushPlus token 写进 `settings.json`。

### 7.3 Claude Code 事件映射
